...
class WindowTypeLauncherItem : public app_list::AppListItemModel {
 public:
  ...
  static void Activate(Type type, int event_flags) {
     switch (type) {
      case TOPLEVEL_WINDOW: {
        ash::shell::ToplevelWindow::CreateParams params;
        params.can_resize = true;
        ash::shell::ToplevelWindow::CreateToplevelWindow(params);
        break;
      }
      case NON_RESIZABLE_WINDOW: {
        ash::shell::ToplevelWindow::CreateToplevelWindow(
            ash::shell::ToplevelWindow::CreateParams());
        break;
      }
      case LOCK_SCREEN: {
        ash::Shell::GetInstance()->session_state_delegate()->LockScreen();
        break;
      }
      case WIDGETS_WINDOW: {
        ash::shell::CreateWidgetsWindow();
        break;
      }
      case EXAMPLES_WINDOW: {

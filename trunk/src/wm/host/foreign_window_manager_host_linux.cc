...
#include "wm/host/foreign_window_manager_host_linux.h"
...
ForeignWindowManagerHostLinux::ForeignWindowManagerHostLinux()
...
bool ForeignWindowManagerHostLinux::Initialize() {
...
void ForeignWindowManagerHostLinux::InitializeForTesting() {
...
aura::RootWindowHost* ForeignWindowManagerHostLinux::CreateRootWindowHost(
...
gfx::AcceleratedWidget ForeignWindowManagerHostLinux::GetAcceleratedWidget() {
  return xwindow_;
}

void ForeignWindowManagerHostLinux::SetDefaultCursor(
    gfx::NativeCursor cursor) {
  XDefineCursor(xdisplay_, xwindow_, cursor.platform());
}

bool ForeignWindowManagerHostLinux::Dispatch(const base::NativeEvent& event) {
  XEvent* xev = event;
...
void ForeignWindowManagerHostLinux::OnWindowInitialized(aura::Window* window) {
  window->AddObserver(this);
}

void ForeignWindowManagerHostLinux::OnRootWindowInitialized(
...
void ForeignWindowManagerHostLinux::OnWindowStackingChanged(
    aura::Window* window) {
  ScheduleConfigure(window->GetRootWindow());
}

void ForeignWindowManagerHostLinux::OnWindowVisibilityChanged(
    aura::Window* window, bool visible) {
  if (window->GetRootWindow())
    ScheduleConfigure(window->GetRootWindow());
}

void ForeignWindowManagerHostLinux::OnWindowBoundsChanged(
    aura::Window* window,
...
void ForeignWindowManagerHostLinux::OnWindowDestroyed(aura::Window* window) {
  window->RemoveObserver(this);
}

void ForeignWindowManagerHostLinux::AddDisplayFromSpec(
...
::Window ForeignWindowManagerHostLinux::GetTopForeignWindow(
...
::Window ForeignWindowManagerHostLinux::FindForeignWindowToStackAbove(
    const aura::Window* window) {
...
void ForeignWindowManagerHostLinux::MapWindowIfNeeded(
...
void ForeignWindowManagerHostLinux::UnmapWindowIfNeeded(
    ForeignWindow* window) {
  ForeignWindow::DisplayState current_display_state =
      window->GetDisplayState();
...
void ForeignWindowManagerHostLinux::RecursiveConfigure(
    aura::Window* window,
    gfx::Vector2d offset,
    ::Window* sibling_to_stack_above,
...
void ForeignWindowManagerHostLinux::Configure() {
...
void ForeignWindowManagerHostLinux::ScheduleConfigure(aura::Window* window) {
  DCHECK(window);
  ...
  if (!configure_pending_) {
    ...
    MessageLoop::current()->PostTask(
        FROM_HERE,
        base::Bind(&ForeignWindowManagerHostLinux::Configure, AsWeakPtr()));
...
ForeignWindowManagerHost* ForeignWindowManagerHost::Create() {
  return new ForeignWindowManagerHostLinux;
}
